...
/**
 * Phase 7.0.4 — single-source-of-truth honesty disclaimer. Embedded
 * verbatim in the JSON `scope` field + the "Scope" section of the
 * Markdown / AsciiDoc exports + (as inline copy) in the dashboard's
 * Security panel. The point is to make it impossible for a
 * downstream reader to mistake an Astraudit audit for CVE coverage
 * or for an authenticated-logic review.
 */
export const SCOPE_DISCLAIMER = {
  what_we_check: [
    "Declared dependencies + lockfile presence (per supported manifest)",
    "Static, rule-based signals from public files: README, LICENSE, SECURITY, CODE_OF_CONDUCT, CONTRIBUTING, CHANGELOG, CI configs, .github/ directory",
    "Repository metadata available via the public GitHub API (stars, push date, license SPDX id, has_wiki, archived, fork)",
    "Optional public-registry metadata for npm / PyPI / crates.io top-level dependencies",
  ],
  what_we_do_not_check: [
    "Transitive CVE scanning — run `npm audit`, `pip-audit`, `cargo audit`, or `bundler audit` instead.",
    "Authenticated logic / auth model / scope checks (private to the running app, not in public files).",
    "Branch protection rules + required reviews + merge queue — not always exposed without admin tokens. Surfaced as 'Unknown' when public surface lacks the data; never guessed at.",
    "Runtime / dynamic analysis — Astraudit is rule-based and static only.",
    "Private repositories — Astraudit is browser-only and free-forever; only public repos are supported.",
  ],
} as const;
...
export function exportToJson(result: AuditResult): string {
  ...
  const out = {
    ...
    generatedAt,
    // Phase 7.0.4 — every export carries the scope disclaimer so a
    // downstream reader (Slack paste, GitHub issue comment, ticket
    // attachment, security dashboard) can't mistake an Astraudit
    // audit for CVE coverage or authenticated-logic review.
...
export function exportToMarkdown(result: AuditResult): string {
  ...
  /* Scope (Phase 7.0.4) --------------------------------------------- */
  lines.push("## Scope of this audit");
  lines.push("");
  lines.push("### What Astraudit checks");
  lines.push("");
  for (const item of SCOPE_DISCLAIMER.what_we_check) {
    lines.push(`- ${escMd(item)}`);
  }
  lines.push("");
  // ALL-CAPS instead of **not** so the pre-existing "no raw markdown
  // markup inside h3 headings" test stays green. Same emphasis, no
  // formatting collision.
  lines.push("### What Astraudit does NOT check");
  lines.push("");
  for (const item of SCOPE_DISCLAIMER.what_we_do_not_check) {
    lines.push(`- ${escMd(item)}`);
  }
  lines.push("");
