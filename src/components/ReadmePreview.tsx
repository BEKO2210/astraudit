    ...
    return (
      <section className="glass overflow-hidden p-5 sm:p-6">
        <header className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-white">{t("panel.readmePreview")}</h3>
        </header>
        <p className="mt-3 text-sm text-slate-300/85">
          {t("readmePreview.noReadme")}
        </p>
        <p className="mt-1 text-[11px] text-slate-500">
          {t("readmePreview.noReadmeHint")}
        </p>
      </section>
    );
  }
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 print:hidden">
          {isTruncated || expanded ? (
            ...
            >
              {expanded ? t("readmePreview.showLess") : t("readmePreview.showMore")}
            </button>
          ) : null}
          <a
            href={`${htmlUrl}#readme`}
          ...
          >
            {t("readmePreview.viewFullReadme")}
            <ExternalLink className="h-3 w-3" />
          </a>
        </div>
      </header>
      <p className="mt-1 text-[11px] text-slate-500">
        {t("readmePreview.renderedSafelyPrefix")}
